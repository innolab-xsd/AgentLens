...
function getIntentTitle(event: SessionEvent): string | undefined {
  const p = payload(event);
  return toStringValue(p.title) ?? toStringValue(p.description);
}

function getFileTarget(event: SessionEvent): string | undefined {
  if (event.kind !== "file_op") return undefined;
  const p = payload(event);
  return toStringValue(p.target) ?? event.scope?.file;
}

function getTokenUsage(event: SessionEvent): {
  total_tokens: number;
  estimated_cost_usd?: number;
} | null {
  const p = payload(event);
...
function buildIntentBuckets(eventsRaw: SessionEvent[]): IntentBucket[] {
  ...
  const byIntent = new Map<string, SessionEvent[]>();

  function inferIntent(event: SessionEvent): string {
...
function isFileCheckEvent(event: SessionEvent): boolean {
...
export function generateFollowupArtifacts(
  ...
  const artifacts: FollowupArtifact[] = targetBuckets
    ...
    .sort(
      (a, b) =>
        b.features.risk_score + b.features.efficiency_waste_score - (a.features.risk_score + a.features.efficiency_waste_score)
    );
...
export function deriveIntentTokenBreakdown(events: SessionEvent[]): TokenBreakdownResult {
  const buckets = buildIntentBuckets(events);
