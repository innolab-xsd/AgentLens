...
function getUsage(event: CanonicalEvent): { total_tokens: number; estimated_cost_usd: number } | null {
...
function inferAgentId(event: CanonicalEvent, fallbackPrefix: string): { id: string; synthetic: boolean } {
  const p = payload(event);
  const explicit =
...
function inferParentId(event: CanonicalEvent): string | undefined {
  const p = payload(event);
  return toStringValue(event.scope?.parent_agent_id) ?? toStringValue(p.parent_agent_id);
}

function inferDelegationId(event: CanonicalEvent): string | undefined {
  const p = payload(event);
  return toStringValue(event.scope?.delegation_id) ?? toStringValue(p.delegation_id) ?? toStringValue(p.task_id);
}

function deriveTokenSplit(events: CanonicalEvent[], usageEvent: CanonicalEvent, usage: { total_tokens: number }) {
...
export function deriveSubagentGraph(eventsRaw: CanonicalEvent[]): SubagentGraphResult {
